...
class CustomEnv(gym.Env):
    def __init__(self,task_node_num=8,interval_node_num=20,vm_num=4) -> None:
        ...
        self.time_error_reward=-30.0
        self.usage_error_reward=-20.0
        self.done_tasks_reward=10.0
        self.done_reward=300.0
        self.inf_replace:np.float32=10000.0
        self.error_break:bool=False
        self.early_begin_reward=50
        self.early_end_reward=50
    ...
    def stateInitModifier(self):
        ...
        while len(split_set)<self.vm_num-1:
            split_set.add(np.random.randint(low=1,high=self.interval_node_num))#是前闭后开
        ...
        # 生成任务的持续时间
        task_computing_time_span=np.random.exponential(scale=1.0/self.mu,size=task_size)
        # 任务的等待时间，保持和之前的代码一致
        task_receive_time_span:np.ndarray= np.random.exponential(scale=1/self.phi,size=task_size)#接收时长
        #TODO 这里应该有问题，不能直接这么写task_process_time_span
        ...
        for index,min_wait_time in enumerate(task_process_time_span):
            while True:
                ...
                if random_wait_time>min_wait_time:
                    ...
                    break
        # 生成任务的截止时刻
        deadline=arrival+task_max_wait_time_span+task_process_time_span
    ...
    def reset(self,task_node_num,interval_node_num,vm_num):
    ...
    def step(self,action:collections.OrderedDict):
        ...
        if (begin_point>=end_point) or (begin_point<arrival_point) or (begin_point<0):
    ...
    def reward(self,intervals_index):
        ...
        # 对早进行的奖励
        reward+=task_usage*(arrival_point-begin_point)*self.early_begin_reward
        # 对提前结束的奖励
        # if deadline_point>end_point:
        reward+=task_usage*(deadline_point-end_point)*self.early_end_reward
        # else:
            # reward-=0
        # 找到对应占用的间隙
        intervals:collections.OrderedDict=self.state['Node']['intervals']
        # 用间隙的占用率进行奖励，尽量占满间隙
        for interval_index in intervals_index:
            reward+=intervals['usage'][interval_index]*(intervals['end'][interval_index]-intervals['start'][interval_index])*30
